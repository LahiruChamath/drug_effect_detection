...
SEQUENCE_LENGTH = 30  # Number of frames to use
N_KEYPOINTS = 33      # MediaPipe pose keypoints
N_FEATURES = N_KEYPOINTS * 2  # (x, y) coordinates = 66
...
def load_sequences(max_length=SEQUENCE_LENGTH):
    """Load raw pose sequences from JSON files"""
    X = []
    y = []
    skipped = 0
    successful = 0
    ...
    for label in CLASSES:
        ...
        for filename in files:
            ...
            try:
                ...
                # Extract pose keypoints from each frame
                sequence = []
                ...
                for frame in frames_data:
                    ...
                    # Only add frame if we have correct number of keypoints
    ...
    print(f"\n{'='*60}")
    print(f"✅ Successfully loaded: {successful} sequences")
...
def train_lstm():
    ...
    print(f"\n✅ Model saved: {MODEL_PATH}/drug_classifier_lstm.h5")
    
    # Compare with SVM/XGBoost
    print("\n" + "=" * 60)
    ...
    if test_accuracy > 0.85:
        print("\n🎉 LSTM OUTPERFORMS baseline models!")
    elif test_accuracy > 0.80:
        print("\n✅ LSTM performance is competitive")
    else:
        print("\n⚠️ LSTM underperforming (likely due to small dataset)")
        print("💡 Consider collecting more data or using SVM/XGBoost for now")
